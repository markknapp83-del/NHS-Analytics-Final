#!/usr/bin/env python3
"""
FINAL Phase 3 Priority Data Refresh - RTT, A&E, Diagnostics
Final corrected version that includes trust names and handles database constraints.
"""
...
class FinalPhase3Refresh:
    """
    Final corrected version of Phase 3 data processing with trust name mapping
    """
    ...
    def _setup_logging(self) -> logging.Logger:
        ...
        logger = logging.getLogger('final_phase3_refresh')
    ...
    def _load_trust_mapping(self) -> Dict[str, str]:
        ...
        try:
            ...
            self.logger.info(f"✅ Loaded {len(trust_mapping)} trust mappings")
            return trust_mapping

        except Exception as e:
            self.logger.warning(f"⚠️  Could not load trust mapping: {e}")
            return {}

    def _get_trust_name(self, trust_code: str, provider_name: str = None) -> str:
        """Get trust name for a trust code, with fallback options"""

        # First try the mapping file
        if trust_code in self.trust_mapping:
            return self.trust_mapping[trust_code]

        # If provider name is available from the data, use it
        if provider_name and provider_name.strip() and provider_name != 'nan':
            return str(provider_name).strip()

        # Last resort: generate a placeholder name
        return f"Unknown Trust ({trust_code})"
    ...
    def _update_trust_metrics(self, trust_code: str, trust_name: str, period: str, data_field: str, data: dict) -> bool:
        """Update specific data field in trust_metrics table with trust name"""
        try:
            ...
            if existing.data:
                ...
                result = self.supabase.table('trust_metrics').update({
                    data_field: data,
                    'trust_name': trust_name,  # Ensure trust name is always updated
                    'updated_at': 'now()'
    ...
    def process_rtt_data(self) -> Dict[str, Any]:
        ...
        # Process only first 2 files for now to avoid timeout
        for file_path in rtt_files[:2]:  # LIMIT TO 2 FILES FOR TESTING
            try:
                ...
                for trust_code, trust_data in trust_groups:
                    ...
                    # Get trust name from Provider Name or mapping
                    provider_name = trust_data['Provider Name'].iloc[0] if 'Provider Name' in trust_data.columns and not trust_data.empty else None
                    trust_name = self._get_trust_name(trust_code, provider_name)
                    ...
                    # Update database with trust name
                    ...
                    if success:
                        ...
                        self.logger.info(f"  ✅ {trust_code} ({trust_name[:30]}...) RTT data updated ({len(specialties)} specialties)")
                    else:
    ...
    def process_ae_data(self) -> Dict[str, Any]:
        ...
        for file_path in ae_files[:2]:  # LIMIT TO 2 FILES FOR TESTING
            try:
                ...
                for _, row in df.iterrows():
                    ...
                    trust_name = self._get_trust_name(org_code, org_name)
    ...
    def run_final_refresh(self) -> Dict[str, Any]:
        """Run final Phase 3 data refresh"""
        start_time = datetime.now()
        self.logger.info("🚀 Starting FINAL Phase 3 Priority Data Refresh")
        self.logger.info(f"Start time: {start_time}")

        # Process in priority order (LIMITED FOR TESTING)
        print("\n" + "="*60)
        print("PHASE 3.2: RTT DATA PROCESSING (HIGH PRIORITY) - LIMITED TO 2 FILES")
        print("="*60)
        rtt_results = self.process_rtt_data()

        print("\n" + "="*60)
        print("PHASE 3.3: A&E DATA PROCESSING - LIMITED TO 2 FILES")
        print("="*60)
        ae_results = self.process_ae_data()

        # Skip diagnostics for now to test the fixes
        print("\n⏭️  Skipping diagnostics for this test run")
        ...
        # Final summary
        print("\n" + "="*60)
        print("🎉 FINAL PHASE 3 TEST REFRESH COMPLETE")
        print("="*60)
...
def main():
    ...
    try:
        ...
        processor = FinalPhase3Refresh()

        # Run final refresh
        results = processor.run_final_refresh()

        # Save results
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        with open(f'final_phase3_test_results_{timestamp}.json', 'w') as f:
            json.dump(results, f, indent=2, default=str)

        print(f"\n💾 Results saved to: final_phase3_test_results_{timestamp}.json")
